...
---Merge keys from source into destination; nested plain tables (not WoW UI objects) are merged recursively.
...
function Utils:CreateScrollFrame(config)
  ...
  local mergedScrollConfig = {}
  self:TableMergeConfig(mergedScrollConfig, {
    scrollSpeedHorizontal = 20,
    scrollSpeedVertical = 20,
  })
  self:TableMergeConfig(mergedScrollConfig, config or {})
