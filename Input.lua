...
function Input:CreateCheckbox(options)
  ...
  input.icon:SetTexture(addon.Constants.media.IconCheckmark)
  ...
  function input:Update()
    addon.Utils:SetBackgroundColor(input, 0.1, 0.1, 0.1, 1)
...
function Input:CreateDropdown(options)
  ...
  input.button.icon:SetTexture(addon.Constants.media.IconCaretDown)
  ...
  -- input.list = addon.Utils:CreateScrollFrame(input:GetName() .. "List", UIParent)
  input.list = addon.Utils:CreateScrollFrame({
    name = input:GetName() .. "List",
    parent = UIParent,
  })
  ...
  function input:GetValueText()
    ...
    local item = addon.Utils:TableGet(input.items, "value", input.value)
  ...
  function input:Update()
    ...
    addon.Utils:TableForEach(input.list.content.items, function(itemButton) itemButton:Hide() end)
    addon.Utils:TableForEach(input.items, function(item, index)
      ...
      if not itemButton then
        ...
        itemButton.iconCheck:SetTexture(addon.Constants.media.IconCheckmark)
      ...
      itemButton:SetScript("OnEnter", function()
        ...
        addon.Utils:SetHighlightColor(itemButton, 1, 1, 1, 0.05)
      end)
      itemButton:SetScript("OnLeave", function()
        if item.value == value then return end
        addon.Utils:SetHighlightColor(itemButton, 1, 1, 1, 0)
      end)

      if item.value == value then
        addon.Utils:SetBackgroundColor(itemButton, 1, 1, 1, 0.08)
        itemButton.iconCheck:Show()
      else
        addon.Utils:SetBackgroundColor(itemButton, 1, 1, 1, 0)
        itemButton.iconCheck:Hide()
      end
...
function Input:CreateColorPicker(options)
  ...
  input.config = CreateFromMixins(
    {
      parent = UIParent,
      value = addon.Constants.colors.primary,
      size = 16,
    }, options or {}
  )
  input.value = input.config.value
  ...
  function input:Update()
    addon.Utils:SetBackgroundColor(input, 1, 1, 1, 1)
