...
-- TODO: Automate pulling this from CHANGELOG.md with a workflow task
local log = [===[# Changelog
...
---Parse and return the log
---@return string[] lines
---@return integer number of versions
function Changelog:GetLog()
  local parsedLines = {}
  local numVersions = 0
  local logLines = strsplittable("\n", log)

  for i = 1, #logLines do
    local line        = logLines[i]
    local isTitle     = string.find(line, "^# ")
    local isVersion   = string.find(line, "^## ")
    local isSubHeader = string.find(line, "^### ")
    local isEmpty     = string.len(line) == 0

    do
      -- Remove bullet points
      line = string.gsub(line, "^- ", "")
      -- Remove inline links
      line = string.gsub(line, "(%b[])(%b())", function(text, _) return text:match("%b[]"):sub(2, -2) end)
    end

    if isVersion then
      numVersions = numVersions + 1
      table.insert(parsedLines, string.sub(line, 4))
    elseif not isTitle and not isSubHeader and not isEmpty then
      table.insert(parsedLines, line)
    end
  end

  return parsedLines, numVersions
end
